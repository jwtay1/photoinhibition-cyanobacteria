...
for iTrack = 1:numel(leafIDs)
    ...
    if strcmpi(lineageData(newIdx).Type, 'wt')
        ...
        %if max(lineageData(newIdx).MeanChl) > 5200
        if lineageData(newIdx).Productivity(end) > 8.5
            ...
            figure(2)
            subplot(1,2,1)
            plot(obj.FileMetadata.Timestamps(frames)/3600, combinedCy5, ...
                'Color', 'red')
            hold on
            
            figure(3)
            subplot(1,2,1)
            plot(obj.FileMetadata.Timestamps(frames)/3600, combinedRFP, ...
                'Color', 'red')
            hold on
